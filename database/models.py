...
class Premia(db.Model):
    ...
    def __repr__(self):
        return f"Premia(id_mech={self.id_mech}, miesiac={self.miesiac}, rok={self.rok}, wysokosc_premii={self.wysokosc_premii})"
...
class Uzytkownik(db.Model):
    ...
    def __repr__(self):
        return f"Uzytkownik(id_uz={self.id_uz}, imie='{self.imie}', nazwisko='{self.nazwisko}', nr_telefonu='{self.nr_telefonu}', adres_mailowy='{self.adres_mailowy}', skrot_hasla='{self.skrot_hasla}', typ='{self.typ}')"
...
# def arc_fkarc_2_klient(mapper, connection, target):
#     session = get_session()
